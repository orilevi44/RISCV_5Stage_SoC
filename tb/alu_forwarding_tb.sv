...
module alu_forwarding_tb ();
    ...
    // Hex file is passed at runtime: +HEX_FILE=sim/hex/memory.hex
    // -------------------------------------------------------------------------
    initial begin
        string hex_file;
        $dumpfile("sim/waves.fst");
        $dumpvars(0, alu_forwarding_tb);
        rst_n       = 0;
        uart_rx_pin = 1'b1;
        ...
        if ($value$plusargs("HEX_FILE=%s", hex_file))
            ...
            $display("[ERROR] No HEX_FILE provided. Use +HEX_FILE=sim/hex/memory.hex");
            $finish;
        end
     end
     ...
     initial begin
        ...
        $display("===========================================");
        $display("           Forwarding and Hazard Units Test");
        $display("===========================================");


        check_gpio(32'd8, "Test 1: add 5+3 one after one");
        check_gpio(32'd6, "Test 2: sub 10-4 ");
        check_gpio(32'd14, "Test 3: load from the memory and use it");
